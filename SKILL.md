...
## 目标兼容层[Goal Compatibility Layer]

Task Driver 必须能把目标定义[Target]映射到宿主 agent 的目标模式[Goal Mode]，但不得让目标模式替代 Task Driver 的 spec、plan、ledger、verification 和 User Acceptance Gate。

目标提供方[goal_provider] 取值：

- `codex`：环境提供 `get_goal`、`create_goal`、`update_goal` 等 Goal 工具时使用。
- `claude-code`：环境支持 Claude Code `/goal` 时使用，产出可执行的 `/goal <condition>` 字符串。
- `ledger-only`：没有可调用目标模式时使用，GoalDraft 只写入 ledger，Task Driver 自己维持目标契约。

生命周期规则：

1. brainstorming 阶段不得创建或替换 Goal；只能形成 Target 草案。
2. planning 阶段在 approved spec 后生成目标草案[GoalDraft]，并写入 plan 或 ledger。
3. executing 启动前才允许激活 Goal：Codex 使用 Goal 工具；Claude Code 输出 `/goal` 条件并要求用户或宿主执行；无能力时降级为 `ledger-only`。
4. 已存在 active Goal 且 target_id 不一致时，必须停机回问；不得自动覆盖。Claude Code 新 `/goal` 会替换旧目标，因此尤其禁止静默发送替换命令。
5. verification 通过前不得标记 Goal complete；Goal 达成不等于用户已验收。User Acceptance Gate 仍是最终用户确认。
6. Goal 工具不可用、hook 被禁用、版本不支持或宿主不暴露控制面时，不得失败退出；写入 `goal_provider: ledger-only` 和降级原因后继续 Task Driver 流程。
7. 目标检测[goal_detection]必须隔离上下文使用子智能体[subagent]或等价 isolated verifier；当前执行上下文不得自证 Goal complete。检测实现可降级为新会话、外部工具或人工隔离审查，但上下文隔离不可降级；没有任何隔离检测路径时，不得标记 Goal complete。

GoalDraft 必须由 Target 派生：

- `target_statement` -> outcome。
- `success_definition` 与 acceptance_criteria -> completion_condition / verification_surface。
- `scope_denominator` -> boundaries。
- `target_principles` 与 constraints -> constraints。
- `stop_or_loop_conditions` -> blocked_stop_condition。
- execution-verification loop 规则 -> iteration_policy。
- isolated subagent 检测要求 -> goal_detection。
...
**Packet 展示规则**：向用户展示 SpecPacket、PlanPacket、TaskResult、ReviewReport、VerificationReport 时，必须将所有英文字段名和枚举值转换为 `中文[英文]` 格式，并使用分组表格或清单展示，禁止直接把 YAML/JSON 风格机器契约贴给用户。转换规则见 `references/packet-templates.md` 的"面向用户展示规则"段落。

阶段交接、停机回问、用户验收门、计划前确认和评估类结论展示，只要包含 packet 字段、状态值、`go/no_go` 结论、`evaluation_result`、`decision_trace` 或扩展字段，也视为用户可见 Packet 展示，必须先渲染成中文分组表格或清单，再给出下一步选项。
