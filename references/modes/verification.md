...
- `pre_acceptance_self_check` 必须包含 GoalDraft / 目标提供方[goal_provider]审计：target_id 是否一致、GoalDraft.status、goal_provider、activation 结果、completion 证据是否来自当前 VerificationReport。
- `isolated_goal_detection`：目标检测[goal_detection]结果；必须来自隔离上下文的子智能体[subagent]或等价 isolated verifier，不能由当前执行上下文自证。
...
## 目标完成门[Goal Complete Gate]

VerificationReport 通过前，不得把 GoalDraft.status 写为 `complete`，不得调用 Codex `update_goal(status=complete)`，不得把 Claude Code goal 自动 clear 解释为用户已验收。

允许完成 Goal 的必要条件：

- coverage[] 中每个 AC 至少达到当前质量层级要求的证据强度。
- target_coverage[] 覆盖 scope_denominator 全部目标单元。
- pre_acceptance_self_check 无 fail。
- GoalDraft.outcome、completion_condition、boundaries、constraints 均与当前证据一致。
- isolated_goal_detection.status 为 pass，且 evidence_refs 指向当前 VerificationReport.coverage / target_coverage 中的证据。
- 未满足项为空，或满足 Partial 进入 User Acceptance Gate 的条件并已明确标记 partial。

若 provider 为 `codex`，上述条件满足后才允许调用 `update_goal(status=complete)`。若 provider 为 `claude-code`，goal 自动 clear 后仍必须完成本节审计。若 provider 为 `ledger-only`，只更新 ledger 中 GoalDraft.status。

目标检测执行规则：

- 必须使用独立 subagent / isolated verifier；检测者只能接收 SpecPacket、GoalDraft、PlanPacket、ledger evidence、VerificationReport 草稿、必要文件路径和命令输出。
- 降级允许从 subagent 降级为新会话 verifier、外部工具审计或人工隔离审查，但隔离上下文不可降级；任何降级都必须写入 isolated_goal_detection.verifier 和 finding。
- 检测者不得接收当前执行上下文的推理过程、未证实意图或“我认为已完成”的结论。
- 检测问题固定为：GoalDraft 的 outcome、completion_condition、boundaries、constraints 是否均被当前证据证明。
- 没有任何隔离检测路径、检测失败、检测证据不足或检测上下文污染时，不得进入 User Acceptance Gate；必须回到 executing、plan-revision、brainstorming 或 blocked。
