...
- Approved spec，可内联输入或位于 `.task-driver/specs/`；进入 plan 编写前，SpecPacket 必须已持久化到 ledger、spec `## SpecPacket` 或 planning handoff。
...
- Approved spec 必须包含目标定义[Target] 或等价字段：target_id、target_statement、success_definition、scope_denominator、target_principles、quality_level、stop_or_loop_conditions。缺失时回到 brainstorming。
- Approved spec 必须已经闭合拆解轴、拆解粒度和功能级检验口径。若只确认了宏观 Phase、文件名、产物标题或“补齐标准”这类粗粒度方向，必须回到 brainstorming 继续拆解。
...
计划中的每个技术任务都必须能追溯到 Target、Decision Trace、Acceptance Criteria 或 Constraints。若一个任务本质是“选择技术方案”“调研后决定路线”“看情况实现 A/B”“定义评分口径”“确认维度标准”，说明拷问没有闭合，必须回到 brainstorming；plan 只能执行已确认方案，不能继续替用户做方案决策。

planning 阶段只负责把已确认的目标和方案拆成可执行契约。若发现目标、范围分母、目标原则、拆解轴、拆解粒度、非目标、AC、质量层级、技术取舍、风险边界、共享理解或验收方式任一项未闭合，必须回到 brainstorming；不得在 plan 中自行补默认值。
...
保存到 `.task-driver/plans/YYYYMMDD-HHmm-主题.md`。

必须包含：

- target_id、目标和 spec 路径。
- 目标达成定义：用 1-3 句说明完成后什么外部行为、文档状态、质量状态或可观察结果成立；不得只有任务列表。
- 目标覆盖矩阵：列出 scope_denominator 中每个目标单元是否进入计划、由哪个 T-NNN 执行、由哪个验证项证明。
- 拆解说明：写明采用的拆解轴、层级、每层产物和每层验收，证明不是从目标直接跳到文件或 Phase。
- 验证方案摘要：在任务清单前先说明最终要运行/检查的功能级验证类型、覆盖哪些 AC、证据强度上限和无法自动验证的人工证据。
- 从 spec 复制的全局约束。
...
- 任务拆分：每个任务能独立验证和评审；不得只有 Phase、阶段标题、文件标题或产物名。
- 行为变化：TDD 步骤、预期失败、最小实现、通过命令、重构边界。
- 非代码变化：确定性的验证步骤。
- 每个任务的 review gate。
- ledger 路径：`.task-driver/ledgers/`。
...
Plan 任务必须达到以下粒度，缺一项即不得进入 executing：

- 目标映射：引用 target_id、scope_denominator 单元、AC-N 或 Constraints。
- 文件范围：精确列出允许创建、修改、测试或文档路径。
- 行为或内容变化：说明要改变什么可观察行为、协议条款、schema、命令输出、UI 流程或文档语义。
- 验证方式：写出功能级命令、样例、脚本、人工审查步骤或反例检查；文件存在和文本命中默认只能作为辅助证据。
- 预期结果：验证通过时应看到什么，失败时如何路由。
- Review Gate：明确检查目标覆盖、功能证据、范围漂移和反偷懒风险。

禁止把以下内容当作可执行任务：

- `Phase 1：补齐治理`。
- `为 stage2 建立 Rubric`，但未列评价对象、维度、尺度、阈值、证据和失败反例。
- `完善各阶段 SKILL.md`，但未逐项说明每个阶段要改变的行为和验证方式。
- `检查文件已写入`，但目标是功能、迁移、流程或协议行为改变。

若一个粗任务可继续拆成会影响目标、AC、验证或风险的子问题，必须先拆；其中需要用户拍板的子问题回到 brainstorming。
...
**Plan version:** v1
**Predecessor:** 无（首版） | .task-driver/plans/YYYYMMDD-HHmm-主题.md（v[N-1] 路径）
...
|---|---|---|---|
| [scope_denominator 单元] | T-001 | AC-1 / Verification Plan 第 1 项 | planned |

## Decomposition Strategy
- 拆解轴：[阶段 / 模块 / 风险 / 用户路径 / 产物类型 / 协议层级 / 问题类型]
- 拆解层级：[Phase -> Task -> Step -> Verification]
- 粒度下限：[文件 + 行为/内容变化 + AC 引用 + 功能级验证]
- 用户已确认的未决设计点处理：[无 / ASM-N / 回到 brainstorming]

## Verification Strategy
- [最终功能级验证类型、命令或检查方式、覆盖 AC-N、预期 evidence_strength]
...
**Target units:** [引用 scope_denominator 中的目标单元]
**Review gate:** target coverage + spec compliance + functional evidence + code/content quality
...
## Verification Plan
- [最终功能级命令或样例、预期结果、覆盖的 AC-N 和目标单元列表、预期 evidence_strength]
...
- `## Goal`、`## Success Definition`、`## Target Coverage Matrix`、`## Decomposition Strategy` 和 `## Verification Strategy` 必须先于任务清单出现，且不能与任务步骤重复成同义列表。
- 每个 Task T-NNN 必须包含 Target units、Files、Acceptance、功能级 Verification 或明确证据强度上限；缺失时回到 planning 修正。
- 搜索占位词。
- 检查任务顺序、接口名称、路径一致。
...
- 检查是否存在“文件存在 / 文本命中 / 已写入”冒充功能级验证；除纯静态低风险文档任务外，命中即修正验证策略。
