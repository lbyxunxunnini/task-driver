...
完整数据收集[grilling_state]表格默认只写入 ledger 或 spec；只有在用户要求、上下文恢复、阻塞诊断、共享理解确认门或 agent 需要解释回退原因时，才面向用户展示完整表格。不得每轮把完整内部状态表格作为默认输出。
...
- 每条 AC 必须优先定义功能级验证。只有纯静态文档或低风险文字修正，才允许把文件存在或文本命中作为主要证据；否则这类证据只能标 weak。
