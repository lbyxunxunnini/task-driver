...
```yaml
spec_packet:
  spec_path: .task-driver/specs/20260707-1200-admin-api-authz.md
  goal: "修复管理员 API 权限绕过"
  target:
    ...
    scope_denominator: ["非管理员拒绝路径", "管理员成功路径", "公共 API contract", "发布前构建与回滚说明"]
    target_principles: ["安全正确性优先于速度", "不得跳过集成测试", "不得改变公共 API contract"]
...
goal_draft:
  ...
  outcome: "非管理员无法访问管理员 API，管理员正常路径不回归。"
  completion_condition: "安全、权限、回归、发布前验证全部有 strong evidence；无 Critical/Important finding。"
  verification_surface:
    - "integration authz test"
    - "integration happy path test"
    - "contract test"
    - "release checklist"
  constraints: ["安全正确性优先于速度", "不得跳过集成测试", "不得改变公共 API contract", "必须记录回滚方案"]
  boundaries: ["非管理员拒绝路径", "管理员成功路径", "公共 API contract", "发布前构建与回滚说明"]
  iteration_policy: "每轮记录改动、证据、未满足项和下一步假设；同一问题最多两轮后路由。"
  blocked_stop_condition: "权限模型或 API contract 变化回到 brainstorming；任务顺序或测试策略错误回到 planning；实现缺陷回到 executing。"
  goal_detection:
    required: true
  ...
  source_packet_ref: .task-driver/specs/20260707-1200-admin-api-authz.md
  status: active
```
...
```yaml
plan_packet:
  plan_path: .task-driver/plans/20260707-1200-admin-api-authz.md
  ledger_path: .task-driver/ledgers/20260707-1200-admin-api-authz.md
  plan_version: v1
  predecessor: 无
  gate_mode: strict
  execution_mode: multi-agent-review
  target_coverage_matrix:
    - target_unit: "非管理员拒绝路径"
    ...
    - target_unit: "管理员成功路径"
      ...
      status: planned
    - target_unit: "公共 API contract"
      task_ids: [T-002]
      verification_refs: [AC-3]
      status: planned
    - target_unit: "发布前构建与回滚说明"
      task_ids: [T-003]
      verification_refs: [AC-4]
      status: planned
  decomposition_strategy:
    axis: 风险
    levels: "Task -> Step -> Verification -> Review"
    outputs: "权限失败测试、权限修复、contract 回归、发布前验证"
    verification_by_level: "集成测试、contract test、build、release checklist"
    granularity_floor: "文件 + 权限行为变化 + AC 引用 + 功能级安全验证"
  tasks:
    - id: T-001
      owner_role: Implementer
      objective: "新增权限绕过失败测试"
      target_units: ["非管理员拒绝路径", "管理员成功路径"]
    ...
    - id: T-002
      ...
      target_units: ["非管理员拒绝路径", "管理员成功路径", "公共 API contract"]
...
verification_report:
  ...
  isolated_goal_detection:
    ...
    status: pass
    finding: "Authz, contract, and release goal boundaries are proven by current verification evidence."
  unmet_requirements: []
