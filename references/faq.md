...
### Q: 小任务也要走完整流程吗？
不需要。同时满足以下条件可走小任务豁免：一轮内完成、最多改一个非关键文件、不增删移文件、不涉及高风险改动、无明显方案分叉、有明确验证方式。此时 spec/plan 可内联，不创建独立 spec / plan 文件；仍必须保留最小 packet 和验证证据。ledger 优先创建或更新；若不创建独立 ledger，最终报告必须写明 `ledger_path: N/A (inline small task)`、不可恢复风险和替代证据。
