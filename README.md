...
v0.8.4 新增 Goal 兼容层：支持 Codex Goal、Claude Code `/goal` 和 ledger-only 三种目标提供方；新增 GoalDraft、目标检测隔离审计、ledger loop 检查脚本；修复 Packet 阶段交接裸 YAML 展示问题，要求评估结论和下一步决策使用中文分组表格展示。

v0.8.2 扩展"范围锚定"规则：新增上下文信息（日志、错误信息、输出结果、截图等）不得被误解为新场景的防护。用户提供的日志、错误信息等是为当前任务服务的上下文，不得被理解为"新的任务目标"。新增"外部样本范围锚定"规则：当任务涉及外部产物（demo、测试样本等）时，严格区分优化目标和参考样本，防止目标漂移。新增"会话状态维持"机制：Task Driver 一旦触发，在任务完成或用户显式取消前，所有后续消息都按框架处理，无需重复触发词。
