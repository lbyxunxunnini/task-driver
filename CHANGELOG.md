...
- **入口门禁**：固定链路之前新增独立章节，首轮回复必须包含当前环节、已识别事项和当前讨论项，首轮回复和讨论阶段禁止调用 Edit / Write 等修改工具。
- **验收目标分流**：原第四步"讨论验收目标"和第五步"输出并确认验收目标"合并为一个章节。计划确认后直接输出验收目标草案，用户可选"确认执行"跳过逐项讨论，或选"逐项确认"进入逐项讨论子流程。模型拿不准的条目标注"待确认"。
- **子代理独立审查**：验收目标确认时询问是否开启。开启后在主 agent 自检通过后触发，审查维度包含状态、证据、阻断原因和回归破坏。未通过时打回修复后再审，独立上限 3 次。未开启则跳过直接进入最终交付。
- **讨论阶段强制推荐**：单事项讨论协议从"必要时提供选项和推荐"改为每轮必须给出推荐方案和理由，可选方案不足 2 个时说明原因。
...
- **Deferred 工具加载**：`TaskCreate`、`TaskList` 和 `TaskUpdate` 的 schema 未进入当前上下文时，必须先通过 `ToolSearch` 精确加载；参数错误后禁止使用猜测参数机械重试。
- **宿主任务状态闭环**：宿主任务只映射最终验收目标，创建后保存任务 ID，执行和验收达标时同步状态，避免工作完成后任务仍然保持未完成。
- **恢复去重**：中断恢复时先读取并复用已有任务，优先使用任务 ID 恢复映射，禁止因上下文压缩或工具失败重复创建。
- **重复任务清理边界**：发现重复项时先核对有效任务和验收证据，不再用批量标记完成代替清理。

### 验证

- 新增 deferred 目标工具与宿主状态同步对话契约。
- 契约检查覆盖 schema 加载、参数错误停止条件、任务 ID 保存、状态同步和恢复去重。

## v0.9.2 (2026-07-20)

### 修复

- **SkillHub 展示名恢复**：将发布页面使用的 `displayName` 恢复为 `Task Driver`，并加入发布前检查，防止再次被中文标题覆盖。
- **讨论不再被草案替代**：模型逐项讨论并判断是否可以收口，但必须由用户选择继续讨论或输出计划。
- **计划补充实施方案**：顶层计划按已确认需求或问题组织，默认使用纵向内容块，同时展示关键决策和实施方案。
- **验收目标先讨论后输出**：计划确认后先逐项讨论最终状态，用户选择生成前不得直接输出目标草案。
- **验收目标停止扩散**：最终目标只保留与已确认事项对应的必要结果，内部测试、文件和版本工作不再冒充终极目标。
- **全链路来源映射**：需求或问题、计划事项、最终验收目标和目标模式保持可追溯关系；合并或拆分必须说明来源和原因。
- **质量判断收敛**：质量结果只保留“达标 / 未达标”；任何已知瑕疵都必须判为未达标并打回修复。
- **用户验收补位**：Agent 无法可靠验证最终结果时，必须通知用户并提供具体验收步骤、通过条件和反馈要求；结果返回前不得标记完成。
- **任务规模与拆分边界**：改为依据业务目标、权限、风险、交付和回滚关系判断整体推进、维护轻量摘要或拆分；拆分范围必须由用户确认。
- **异常分类与重试收敛**：区分可修复、需要用户验收、需要用户授权或决定、受阻；只有可修复问题进入最多 3 次且每次改变策略的修复循环。
- **统一异常通知**：异常统一说明当前结果、具体原因、已完成内容和下一步；用户操作必须附具体步骤和通过条件，不用笼统提醒或长日志代替。
- **独立 FAQ 与场景指南**：新增常见问题直答，以及低风险修复、新功能、跨模块重构、高风险外部操作、用户验收体验任务五类最佳实践。
- **渐进式文档入口**：README 提供快速判断和导航，Quick Start、FAQ、场景指南与完整协议各自承担独立职责并按需展开。

### 验证

- 新增讨论、计划、验收目标和多轮恢复的对话契约用例。
- 新增任务拆分、异常路由、重试变更、统一通知和文档路由的小型对话契约用例。
- 契约检查覆盖阶段切换门禁、计划实施方案、目标映射、任务规模、异常处理和渐进式文档规则。

## v0.9.1 (2026-07-16)

### 更新概览

- **TRACE 评测对齐到 4.9**：围绕可信任度、可靠性、适用性、规范性和有效性补强高杠杆规则，保持小 skill 形态，不恢复旧重流程。
- **新增 30 秒选择器**：在 `SKILL.md`、`README.md` 和 `references/quick-start.md` 增加“使用 / 不使用 / 谨慎使用”判断，帮助用户快速判断是否进入任务驱动。
- **新增验收质量判断**：自检阶段在“已达成 / 未达成 / 受阻”之外补充“够用 / 有瑕疵 / 不够用”三档质量判断，防止把有风险的结果包装成完美完成。
- **新增高风险复核规则**：涉及生产配置、发布、费用、真实外部调用、删除、覆盖、合并、不可逆数据变化或安全权限时，最终交付必须说明复核结果或用户复核建议。
- **补强异常降级**：目标管理工具创建失败、暂不可用或已有未完成目标冲突时，明确轻量目标降级和用户选择路径。
- **发布状态收敛**：`pm-memory/` 纳入忽略规则，避免本地产品记录污染发布包；契约检查覆盖 30 秒选择器、质量判断和高风险复核，防止后续回退。

## v0.9.0 (2026-07-16)

### 破坏性变更

- **重构为低认知负担链路**：主流程改为“仔细询问 -> 生成计划 -> 讨论计划 -> 生成验收标准 -> 讨论验收标准 -> 创建目标并执行 -> 自检未过则打回修复 -> 无法修复给出结论”。
- **取消默认重文档工件**：不再默认生成多份中间文档；普通任务只在对话中展示计划和验收标准。
- **用户可见输出降噪**：用户可见流程使用中文自然语言，不暴露机器化流程名、状态名和内部字段。
- **验收标准独立讨论**：验收标准必须单独生成、单独讨论、单独确认；模型自拟标准只能作为草案，工程测试只能作为辅助检查。
- **权限边界收紧**：不默认新建分支、提交、推送或合并；使用临时分支完成后必须询问用户是否合并、保留、提交、推送或放弃。

### 改进

- **目标闭环**：最终验收标准确认后必须创建目标或维护轻量目标；目标内容只能来自最终验收标准。
- **自检打回循环**：执行后逐条检查最终验收标准，未达成就打回修复；同一验收项最多打回 3 次，全部达成后才能完成目标。
- **打回分类**：区分实现打回、计划打回和验收打回，防止盲修或擅自改验收。
- **大任务写入规则**：内容很多时先写骨架再分块填充，超过 200 行的新文件或大段内容必须拆成多次写入。
- **询问顺序补强**：仔细询问必须先问为什么做和做成什么样，再问怎么做。
- **单任务单摘要**：大任务、超大任务或长执行只维护一份 `.task-driver/{任务标题}-{YYYYMMDDHHmmss}-计划摘要.md`，防止上下文丢失但不恢复旧重流程。
- **轻量表格展示**：计划和验收标准使用轻量表格展示，支持草案、已确认、执行中、已达成、未达成、受阻等中文状态。

### 修复

- 修复旧流程输出过重、名词过多、澄清过浅、虚假验收标准、执行前文档过量和分支动作越权的问题。
...
- **PlanPacket 面向用户展示简化**：基本信息只保留目标、门禁模式、执行模式、质量层级、计划版本，隐藏 plan_path、ledger_path、target_id、status。任务列表简化为一行一个任务带 AC 引用格式。目标覆盖矩阵和拆解策略隐藏，仅内部交接使用。
- **门禁模式简化**：去掉 standard 和 lite 选项，只保留 strict（严格模式）。
