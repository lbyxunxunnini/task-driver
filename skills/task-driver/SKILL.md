...
- 在 ledger 或当前回复中记录 `mode: degraded-single-skill`、不可用阶段和降级原因。
- 按阶段顺序补齐该阶段的必需输出：SpecPacket、PlanPacket、TaskResult、ReviewReport、VerificationReport。
- 使用对应阶段的自检门禁；无法读取阶段 skill 时，使用主控中的最小门禁。
- 最终报告列出哪些阶段发生降级，以及降级是否影响验证证据强度。
...
| standalone minimal protocol used | 已使用独立最小协议 |
| mode | 执行模式 |
| single-agent | 单智能体模式 |
...
遇到异常时，必须使用结构化模板输出提示，确保用户能快速定位问题类型、原因和下一步动作。完整模板见 `docs/task-driver/error-templates.md`。
