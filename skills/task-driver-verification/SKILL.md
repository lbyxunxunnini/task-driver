...
|---|---|---|
| [验收标准] | `[命令/文件]` -> [结果] | Met / Partial / Not met / Blocked |
```
...
写入 ledger。字段以 `skills/task-driver/SKILL.md` 的结构化交接 Packet 为准；本阶段至少写入 status、mode、逐条 requirement 的 evidence、evidence_strength、coverage、未满足要求及原因。

## 循环退出

验证失败后可以回到执行阶段，但同一 requirement 最多 2 轮。第 2 轮仍失败时，必须输出 `blocked`、`partial` 或 `plan-revision`，不得继续要求执行阶段盲修。
